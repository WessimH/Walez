...
export default function PlatformEventExplorer() {
  ...
  const [loadDuration, setLoadDuration] =
    useState<number | null>(null);
  const [publishDuration, setPublishDuration] =
    useState<number | null>(null);
  ...
  async function loadEvents() {
    ...
    try {
      ...
      if (sortedEvents.length > 0) {
        ...
        setSelectedEventName(firstEventName);
        setFieldValues({});
      }
    } catch (currentError) {
      setEvents([]);
  ...
  async function publishEvent() {
    ...
    if (requiredFieldsMissing.length > 0) {
      setError(
        `Champs obligatoires manquants : ${requiredFieldsMissing
  ...
  async function copyValue(
    ...
    try {
      ...
      setError("Impossible de copier la valeur.");
    }
  }

  function changeSelectedEvent(eventApiName: string) {
    setSelectedEventName(eventApiName);
    setFieldValues({});
    setPublishResult(null);
    setError("");
  }
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-gray-500">
            Consulte les Platform Events de l’org et publie
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end">
          <div className="flex-1">
            ...
            <select
              ...
              onChange={(event) =>
                changeSelectedEvent(event.target.value)
              }
      ...
      {!selectedEvent && !loadingEvents && (
        <section className="rounded-xl border border-dashed border-gray-300 bg-white p-12 text-center">
          ...
          <p className="mt-2 text-sm text-gray-500">
            Clique sur Load events pour récupérer les
            événements de l’org.
          </p>
        </section>
      )}
              ...
              {editableFields.map((field) => {
                ...
                return (
                  <div key={field.apiName}>
                    <label className="mb-2 flex items-center justify-between gap-3 text-sm font-medium text-gray-700">
                      <span>
                        {field.label}
                        {field.required && (
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-3 space-y-2 font-mono text-sm">
                <p>
                  Event: {publishResult.eventApiName}
                </p>
